...
def plot_spectrum(parameters, omega, beta, eta):
    ...
    if (K != 1):
        # calc GF analytically
        green = green_order_0(k_vals, omega, beta[0], K, v_F / (2 * K) , alpha, eta[0])
        
        spectrum_expected_R = omega * np.ones(2 * p_c + 1) - 1 / green[:, 0,0]
        spectrum_expected_L = omega * np.ones(2 * p_c + 1) - 1 / green[:, 1,1]
    
    a1.plot(k_vals, spectrum_expected_R.real, color = "r", linestyle = "dashdot",
            label = "Expected spectrum" ) 
    a1.plot(k_vals, spectrum_expected_R.imag, color = "r", linestyle = "dotted") 
            
    a1.plot(k_vals, spectrum_expected_L.real, color = "r", linestyle = "dashdot")
    #a1.plot(k_vals, spectrum_expected_L.imag, color = "r", linestyle = "dotted")

    #a1.set_xlim([-np.pi, np.pi])
    a1.set_ylim([1.5 * min(spectrum_expected_R.real), 1.5 * max(spectrum_expected_R.real)])
...
def i_0(z, b):
    """Main building block of the csc integrals"""
    return (2**(b-1) * special.gamma(b/2+1j*z/2) * special.gamma(b/2-1j*z/2)
            * np.sin(np.pi*b/2 + 1j*z*np.pi/2) * special.gamma(1-b) / np.pi)


def j_0(k, i_omega_n, beta=1.1, b=0.5, u=1.0):
    """Integral e^(i_omega_n tau - I k x) csc b+1 csc b"""
    return (-1j * u * beta**2 / np.pi**2 * np.sin(np.pi*b)
            * i_0(beta * (i_omega_n - u*k) / (2*np.pi), b+1) 
            * i_0(beta * (i_omega_n + u*k) / (2*np.pi), b) )

        
def green_order_0(k_vals, omega=0.0, beta=1.1, K=0.6, u=1.0, alpha=1e-3, delta=1e-4):
    """Zeroth order perturbation theory for SP Green's function"""
    k_vals = np.asarray(k_vals)
    M = (K + 1/K - 2) / 2
    prefactor = -1 / (2*beta*u) * (np.pi * alpha /(beta*u))**M
    g_rr = prefactor * j_0(-k_vals, omega + 1j * delta, beta, M/2, u)
    g_ll = prefactor * j_0(k_vals, omega + 1j * delta, beta, M/2, u)
